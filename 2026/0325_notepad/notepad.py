...
class Notepad:
    ...
    def _build_menu(self):
        ...
        edit_menu.add_separator()
        edit_menu.add_command(label="찾기/바꾸기...", accelerator="Ctrl+H", command=self.find_replace)
        menubar.add_cascade(label="편집", menu=edit_menu)

        # 서식
        format_menu = tk.Menu(menubar, tearoff=0)
        format_menu.add_command(label="글꼴...", command=self.choose_font)
        self.word_wrap = tk.BooleanVar(value=True)
        format_menu.add_checkbutton(label="자동 줄바꿈", variable=self.word_wrap, command=self.toggle_wrap)
